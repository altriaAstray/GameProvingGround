...
public class AudioProcessor : MonoBehaviour
{
	...
	private int samplingRate = 44100;               // fft 采样频率

	/* 对数频率平均控制 */
	private int nBand = 12;                         //波段数
	public float gThresh = 0.1f;                    //灵敏度

	int blipDelayLen = 16;                          //信号延迟长度
	int[] blipDelay;							    //信号延迟

	private int sinceLast = 0;                      //抑制双拍的计数器

	private float framePeriod;                      //帧周期
	...
	float[] dobeat;
	int now = 0;
	// 上述循环缓冲区的时间索引 time index for circular buffer within above

	float[] spec;                                   //上一步的频谱
													// the spectrum of the previous step

	/* Autocorrelation structure 自关联结构*/
	int maxlag = 100;                               //（以帧为单位）跟踪的最大延迟
													// (in frames) largest lag to track

	float decay = 0.997f;                           //运行平均值的平滑常数
													// smoothing constant for running average
	Autoco auco;

	private float alph;                             //节奏偏差惩罚和起始强度之间的权衡常数
	...
	void Start ()
	{
		...
		//initialize record of previous spectrum 初始化先前频谱的记录
	...
	private class Autoco
	{
		...
		public Autoco (int len, float alpha, float framePeriod, float bandwidth)
		{
			...
			// 计算对数滞后高斯加权函数，以选择 120 bpm 左右的速度
			...
			for (int i = 0; i < del_length; ++i) {
				...
				// 加权在 log-BPM 轴上是高斯的，以 wmidbpm 为中心，SD = woctavewidth octaves
